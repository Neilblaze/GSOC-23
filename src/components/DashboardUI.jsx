...
const useModalInteractions = () => {
  ...
  useEffect(() => {
    // listen for keydown events & trigger interactions
    const handleKeyDown = () => {
...
function DashboardUI({ gesture }) {
  ...
  const { showModal, showModal2, handleModalInteraction } =
  ...
  const enterPin = () => {
    if (pin === "") return showToastOnce("PIN cannot be empty!", "error");
      ...
      return showToastOnce("Wrong PIN", "error"); // Default PIN is 1234
    else {
      ...
      return (
        ...
        >
          ...
          <div className="absolute bottom-1 w-screen text-center mb-4 text-gray-500 text-sm">
            Utilize the 👌 gesture using your <b>Right Hand</b> to redirect to
            the next screen by hovering over the <b>LOGIN</b> card.
          </div>
        </div>
      );
    case 2:
          ...
          <div className="grid grid-cols-4 gap-4 mt-6">
            ...
            {isProcessing ? (
              <div className="h-20 col-span-2 border-indigo-500 border-4 flex items-center justify-center text-2xl font-medium rounded-xl">
                PROCESSING
                <svg
                  className="animate-spin h-5 ml-2 text-white"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 24 24"
                >
        ...
        >
          ...
          {showMiniStatementModal && (
            <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center z-30 bg-opacity-90 bg-black">
              <img
                src="./miniStatemnt.gif"
                alt=""
                className="w-3/5 h-3/5 transition-opacity duration-500 pointer-events-none z-50"
              />
            </div>
          )}
        </div>
      ...
      return (
        ...
        >
          ...
          <div className="absolute bottom-24 flex items-center justify-center w-screen text-gray-500 text-lg">
            <span className="mr-2">
              Press <b>ESC</b> to go back to dashboard
            </span>
            <img
              src="/goBack_icon.png"
              alt=""
